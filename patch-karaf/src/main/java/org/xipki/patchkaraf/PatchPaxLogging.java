...
public class PatchPaxLogging {

  public PatchPaxLogging() {
  }
  ...
  private static int func(String[] args) throws IOException {
    ...
    try {
      ...
      while ((line = reader.readLine()) != null) {
        if (line.startsWith("log4j2.pattern")) {
          // log also the line number
          line = line.replace("%-32c{1}", "%-32c{1} %4L");
          writer.write(line);
        } else {
          writer.write(line);
        }
  ...
  private static int printUsage(String message) {
    ...
    sb.append("\nSYNTAX");
    sb.append("\n\tjava " + PatchPaxLogging.class.getName() + " [options]");
    sb.append("\nOPTIONS");
    sb.append("\n\t--file");
    sb.append("\n\t\tFile to be patched");
    sb.append("\n\t--backup");
